...
- **Actions** — every command Strand exposes: open/clone/switch repository, show any view, fetch/pull/push/sync, stash and snapshot, create branch/tag/remote, interactive rebase, review actions (pin/move/clear baseline, copy feedback as prompt), worktree cleanup, settings and theme, and "Abort <operation>" while a merge or rebase is paused.
...
- **Stashes** — "Apply stash: …" and "Pop stash: …" rows per stash.
