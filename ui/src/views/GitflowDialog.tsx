...
export function GitflowDialog({ path, onClose }: { path: string; onClose: () => void }) {
  ...
  const actionLabel: Record<FlowAction, string> = { start: 'Start workflow', finish: 'Finish workflow', continue_merge: 'Continue merge', abort_merge: 'Abort merge' };
  return <Dialog className="git-tool-dialog" title="Git-flow workflows" size="lg" busy={busy} initialFocusRef={first} onClose={onClose}
    ...
    <div className="clone-body git-tool-body">
      ...
      <button ref={first} className="btn" disabled={busy} onClick={() => void run(async () => { setTool(await tauri.repoGitflowDetect()); })}>Check Git-flow installation</button>
      ...
      {state && <>
        ...
        {state.enabled && <>
          <label className="clone-field"><span className="lbl">Workflow</span><Select className="clone-input" value={kind} disabled={busy} onChange={(e) => { setKind(e.target.value as FlowKind); setName(''); setPlan(null); }}><option value="feature">Feature</option><option value="release">Release</option><option value="hotfix">Hotfix</option></Select></label>
