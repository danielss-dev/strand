...
export function SparseCheckoutDialog({ path, onClose }: { path: string; onClose: () => void }) {
  ...
  async function apply(disable: boolean) {
    ...
    try {
      ...
      setMessage(output || (disable ? 'Sparse checkout disabled. All tracked files restored.' : 'Folder selection updated.'));
  ...
  return <Dialog title="Choose checked-out folders" icon="folder" className="clone-options-dialog" busy={busy} onClose={onClose} footer={<>
    ...
    <div className="clone-body">
      ...
      {!state ? <p>Reading tracked folders…</p> : <>
        <p>{state.enabled ? 'Sparse checkout is enabled.' : 'All tracked folders are included.'}</p>
        <p className="stash-blurb">Choose the folders to keep on this computer. Other files remain in Git and can be restored here later. Git also keeps root files and files alongside the selected folders and their parents.</p>
        {!editable ? <><p>This checkout uses custom file patterns. You can inspect them below, or disable sparse checkout before choosing folders.</p><pre className="sparse-patterns">{state.patterns}</pre></> : <>
          <label className="clone-field"><span className="lbl">Find a folder</span>
            <input ref={search} className="clone-input" value={query} disabled={busy} onChange={(e) => setQuery(e.target.value)} placeholder="Search folders…" />
          </label>
          <span className="lbl">Folders to keep</span>
          <div className="sparse-directory-list" role="group" aria-label="Selection to apply">
            {matches.slice(0, 100).map((dir) => {
              ...
              return <label key={dir}><input type="checkbox" checked={selected.includes(dir)} disabled={busy} onChange={(e) => setSelected((old) => e.target.checked ? [...old, dir] : old.filter((item) => item !== dir))} />
                <span>{dir}</span><small>{selected.includes(dir) || inherited ? 'Included' : partial ? 'Some folders included' : 'Not checked out'}</small>
              </label>;
            })}
            {!matches.length && <p>No matching folders.</p>}
          </div>
          ...
          <label><input type="checkbox" checked={sparseIndex} disabled={busy} onChange={(e) => setSparseIndex(e.target.checked)} /> Use a sparse index (advanced)</label>
