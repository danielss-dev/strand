...
Rebindable defaults include the command palette (`Mod+K`), destinations `Mod+1`–`Mod+7`, Workbench customization (`Mod+8`), push/pull/fetch/sync, the theme toggle, the repo switcher (`Mod+E`), and the AI commit-message suggestion (`Mod+Shift+M`). The full table lives in [Keyboard and palette](keyboard-and-palette.md).
