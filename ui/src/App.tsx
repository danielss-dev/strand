...
export function App() {
  ...
  const paletteActions = useMemo<PaletteAction[]>(() => [
    ...
    { id: 'open',    label: 'Open repository…',  shortcut: '⌘O', run: () => showToast('Open dialog: TODO (use @tauri-apps/plugin-dialog)') },
    { id: 'tweaks',  label: 'Toggle theme',      shortcut: '⌘⇧T', run: () => setSetting('theme', theme === 'dark' ? 'light' : 'dark') },
  ], [setView, selectFile, onSync, showToast, setSetting, theme]);
  ...
  return (
    <div className="os-bg" data-theme={theme} data-density={density} data-platform={platform} style={rootStyle}>
      <div className="strand-window">
        <Topbar onOpenPalette={() => setPaletteOpen(true)} onSync={onSync} syncing={syncing} />
