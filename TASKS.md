...
- ☑ DAN-68 in-progress checkout feedback — sidebar (context menu + row
  activate), palette, topbar branch switcher, commit-graph Checkout, reflog
  detached checkout, create-from-remote / tag detach / stash apply|pop|drop
  reuse `ToastViewport` `networkMessage` via App `setNetProgress` (no `netOpId`)
  and `createLocalGitOpRunner` (`ui/src/lib/localGitOp.ts`).
...
- ☑ Heroi fuller agentic UX: repo-cwd command execution with command+output
  in the grouped tool block, composer paste/attach images for vision
  providers, and catalog/probe for Claude Fable 5.1 (`claude-fable-5-1`)
  plus GPT-6 Astra (`gpt-6-astra`) (`heroi.rs`, `heroi/models.rs`,
  `HeroiView`, `attachments.ts` — DAN-70).
...
- ☑ Proper notification viewport (`ToastViewport`): timed success/error pills,
  ...
  Local checkout / track / tag-detach / graph and reflog checkout / stash
  writes reuse the same `netProgress` / `networkMessage` slot via
  `createLocalGitOpRunner` (DAN-68) without a cancel control; overlapping
  writes toast instead of throwing.
