...
export function BisectDialog({ path, initialRevision, onClose }: { path: string; initialRevision?: { revision: string; rating: 'good' | 'bad' }; onClose: () => void }) {
  const [state, setState] = useState<BisectState | null>(null);
  const [good, setGood] = useState(initialRevision?.rating === 'good' ? initialRevision.revision : '');
  const [bad, setBad] = useState(initialRevision?.rating === 'bad' ? initialRevision.revision : 'HEAD');
  ...
  return <Dialog className="git-tool-dialog" title="Find regression" size="lg" busy={busy} initialFocusRef={first} onClose={onClose}
    footer={<><button className="btn" ref={first} disabled={busy} onClick={() => { setError(''); void refresh(); }}>Refresh from Git</button><button className="btn" disabled={busy} onClick={onClose}>Close and resume later</button></>}>
    <div className="clone-body git-tool-body">
      ...
      {state && !state.active && <>
        ...
        <label className="clone-field"><span className="lbl">Last known working commit</span><input className="clone-input" value={good} disabled={busy} onChange={(e) => setGood(e.target.value)} placeholder="Commit, branch or tag before the problem" /></label>
        <label className="clone-field"><span className="lbl">Commit with the problem</span><input className="clone-input" value={bad} disabled={busy} onChange={(e) => setBad(e.target.value)} /></label>
        <p className="stash-note">Git temporarily checks out commits for you to test. End the search to return to your original checkout. Commit or stash changes before starting or moving to another commit.</p>
        {!state.clean && <p className="clone-error">Commit or stash changes before starting.</p>}
        <button className="btn primary" disabled={busy || !state.clean || !good || !bad} onClick={() => void run(() => tauri.repoBisectStart(path, good, bad, state.token))}>Start search</button>
      </>}
      {state?.active && <section className="git-tool-review" aria-label="Bisect progress">
        <strong>{state.culprit ? 'First broken commit found' : state.ambiguous ? 'Result is ambiguous — skipped commits remain' : 'Test this revision'}</strong>
        ...
        <div className="git-tool-actions">
          <button className="btn primary" disabled={busy || !!blocked} onClick={() => rate('good')}>Mark working{state.good_term !== 'good' ? ` (${state.good_term})` : ''}</button>
          <button className="btn" disabled={busy || !!blocked} onClick={() => rate('bad')}>Mark broken{state.bad_term !== 'bad' ? ` (${state.bad_term})` : ''}</button>
        ...
        <button className="btn danger" disabled={busy || !state.clean} onClick={() => reset ? rate('reset') : setReset(true)}>{reset ? 'Confirm return to original checkout' : 'End search and return…'}</button>
